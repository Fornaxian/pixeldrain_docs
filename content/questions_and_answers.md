...
hold this data are set to 16 MiB by default on Linux. Unfortunately that is
often too small when sending files over a high latency or high loss connection.
The optimal size for these buffers can be calculated using the [bandwidth-delay
...
* Spain (since 2025-06). They did not provide a reason for this block. The fake
  certificate which is served seems to be signed by a company called
  [Allot](https://www.allot.com/). I did reach out but they did not reply.
